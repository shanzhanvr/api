 ...
 * Time: 12:01
 */
namespace App\Api\V1\Controllers\Pay;
use App\Api\V1\Bls\Merchant\Model\AccountModel;
use App\Api\V1\Bls\Merchant\Model\RechargeModel;
use App\Api\V1\Controllers\BaseController;
use App\Bls\Merchant\Model\TradeRecodeModel;
    ...
    public function checkOrder(Request $request){
        ...
        if(!empty($model)){
            if($model->status == StatusConst::TRADE_SUCCESS){//充值成功
                return JsonResponse::success(['status'=>StatusConst::TRADE_SUCCESS]);
            }else if($model->status == StatusConst::TRADE_FAIL){//充值失败
                return JsonResponse::success(['status'=>StatusConst::TRADE_FAIL]);
            }else{
                return JsonResponse::success(['status'=>StatusConst::TRADE_ING]);
            }
        }
        return JsonResponse::success(['status'=>StatusConst::TRADE_ERROR]);
    }
    /**
     * @author jason
     * @desc 下单
