...
class CollectFeesController extends BaseController{
    ...
    public function scenelCollPay(){
        ...
        if(!isset($input['secenlCollId']) && empty($input['secenlCollId'])){
            return JsonResponse::error(0,'参数错误');
        }
        $scenelColl = ScenelCollectModel::find($input['secenlCollId']);
        if(empty($scenelColl)){
            return JsonResponse::error(0,'该功能已关闭或不存在');
        }
        //判断当前该账户余额是否充足
        if(empty($this->merchant->account) || $this->compareAmount($this->merchant->account->blance,$scenelColl->price) =='-1'){
